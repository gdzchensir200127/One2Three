...
class TriMesh:
    ...
    normals: Optional[cp.ndarray] = None

    faces_normal: Optional[cp.ndarray] = None

    # Extra data per vertex and face.
    vertex_channels: Optional[Dict[str, cp.ndarray]] = field(default_factory=dict)
    face_channels: Optional[Dict[str, cp.ndarray]] = field(default_factory=dict)
    ...
    def load_from_obj(cls, f: Union[str,BinaryIO]) -> "TriMesh":
        ...
        if isinstance(f, str):
            ...
            return cls(
                ...
                faces=cp.array(faces),
                normals=cp.array(normals) if len(normals)>0 else None,
                faces_normal=cp.array(faces_normal) if len(faces_normal)>0 else None,
    ...
    def write_ply(self, raw_f: BinaryIO):
        write_ply(
            ...
            coords=self.verts,
            rgb=(
                cp.stack([self.vertex_channels[x] for x in "RGB"], axis=1)
