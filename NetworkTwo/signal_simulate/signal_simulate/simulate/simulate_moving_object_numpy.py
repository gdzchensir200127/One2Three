...
def frame_simulate(mesh:TriMesh, radar_pos:np.ndarray) -> np.ndarray:
  ...
  vert1 = np.array([verts[i-1] for i in faces[:,0].tolist()])
  vert2 = np.array([verts[i-1] for i in faces[:,1].tolist()])
  vert3 = np.array([verts[i-1] for i in faces[:,2].tolist()])
  vec1 = vert2-vert1
  vec2 = vert3-vert1
  normal_vec = np.cross(vec1, vec2)
  normal_vec_norm = np.linalg.norm(normal_vec,axis=1)
  ...
  signal_vec_norm = np.linalg.norm(signal_vec,axis=1)

  dot_product = np.sum(normal_vec*signal_vec,axis=1)
  angle = np.arccos(dot_product/(normal_vec_norm*signal_vec_norm))
  
  theta = np.where(angle<np.pi/2 ,angle ,np.pi-angle)*2
  R = np.linalg.norm(signal_vec,axis=1)  # distance
  Tp = 2*R/light_speed  # TOF
  Tp = np.repeat(Tp.reshape(-1,1),bins,axis=1)
  # Q component
  ...
  # signal strength
  sigma_square = 40
  orientation_factor = np.exp(-(theta**2)/(2*sigma_square))
  ...
  receive_signal = np.sum(signal,axis=0)
...
def merge_signal(process_num,result_queue:Queue,lock:Lock):
  ...
  savemat('/home/muxin/hdd/signal_simulate/data/simulate_data/display_60_np.mat',{'data':signal})
