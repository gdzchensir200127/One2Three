...
def randomRotation():
	...
	pos *= np.linalg.norm(np.array([0,0.5,0.6]))
	phi = np.arcsin(pos[2])
...
def getFixedViews(FIXED):
	if FIXED==4:
		# camPosAll = np.array([[1e-7,0.5,0.60],[0.5,1e-7,0.60],[1e-7,-0.5,0.60],[-0.5,1e-7,0.60]])
		camPosAll = np.array([[1e-7,0.74,0.10],[0.74,1e-7,0.10],[1e-7,-0.74,0.10],[-0.74,1e-7,0.10]])
	# if FIXED==4:
