...
class MeshRenderer(object):
    ...
    def _render_pointcloud(self, vertices: np.ndarray, width, height,
        ...
        alpha = np.zeros((height, width, 3), np.float64)
        whiteboard = np.ones((3, height, width), np.float64)
    ...
    def visualize_reconstruction_s2p(self, gt_coord, coord, image_shape = (256,256)):
        camera_k = np.array([[self.camera_f[0], 0, self.camera_c[0]],
                             [0, self.camera_f[1], self.camera_c[1]],
                             [0, 0, 1]])
        # inverse y and z, equivalent to inverse x, but gives positive z
        ...
        tvec = np.zeros(3, dtype=np.float32)
        dist_coeffs = np.zeros(5, dtype=np.float32)
        gt_pc, _ = self._render_pointcloud(gt_coord, image_shape[1], image_shape[0],
                                           camera_k, dist_coeffs, rvec, tvec)
        pred_pc, _ = self._render_pointcloud(coord, image_shape[1], image_shape[0],
                                             camera_k, dist_coeffs, rvec, tvec)
    ...
    def s2p_batch_visualize(self, batch_input, batch_output, atmost=3):
        ...
        for i in range(batch_size):
            gt_points = batch_input["points"][i].cpu().numpy() + mesh_pos
            coord = batch_output['pred_coord'][i].cpu().numpy() + mesh_pos
            images_stack.append(self.visualize_reconstruction_s2p(gt_points,coord))
        return torch.from_numpy(np.concatenate(images_stack, 1))