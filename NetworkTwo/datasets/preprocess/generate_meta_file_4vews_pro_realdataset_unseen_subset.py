...
def process_category(category:str, percent):
    global train_all_list, test_all_list, all_list
    root_dir = os.path.join(dataset_path,category)
    train_meta_file = os.path.join(meta_dir,"train_tf_" + category + f"_S2M_4views_pro_realdataset_unseen_percent_{percent}.txt")
    val_meta_file = os.path.join(meta_dir,"val_tf_" + category + f"_S2M_4views_pro_realdataset_unseen_percent_{percent}.txt")
    test_meta_file = os.path.join(meta_dir,"test_tf_" + category + f"_S2M_4views_pro_realdataset_unseen_percent_{percent}.txt")
    file_list = []
    ...
    train_list = file_list[:4 * train_block]
    val_list = file_list[4 * train_block:4 * (train_block+val_block)]
    test_list = file_list[4 * (train_block+val_block):4 * (train_block+val_block+test_block)]
    train_list = shuffle_in_blocks(train_list, 4)
    val_list = shuffle_in_blocks(val_list, 4)
    test_list = shuffle_in_blocks(test_list, 4)
    train_list = train_list[:int((len(train_list)//4)*percent//100)*4]
    val_list = val_list[:int((len(val_list)//4)*percent//100)*4]
    test_list = test_list[:int((len(test_list)//4)*percent//100)*4]
...
def process_all(percent):
    global all_list, train_all_list, test_all_list
    train_meta_file = os.path.join(meta_dir,f"train_tf_all_S2M_4views_pro_realdataset_unseen_percent_{percent}.txt")
    val_meta_file = os.path.join(meta_dir,f"val_tf_all_S2M_4views_pro_realdataset_unseen_percent_{percent}.txt")
    test_meta_file = os.path.join(meta_dir,f"test_tf_all_S2M_4views_pro_realdataset_unseen_percent_{percent}.txt")
    with open(train_meta_file, 'w') as file:
