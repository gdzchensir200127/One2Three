...
def process_category(category:str):
  ...
  train_meta_file = os.path.join(meta_dir,"train_tf_" + category + "_S2M_4views.txt")
  test_meta_file = os.path.join(meta_dir,"test_tf_" + category + "_S2M_4views.txt")
  dir_list = []
  ...
  with open(train_meta_file, 'w') as file:
    for item in dir_list[:4*len(dir_list)//5]:
...
def process_all():
  train_meta_file = os.path.join(meta_dir,"train_tf_all_S2M_4views.txt")
  test_meta_file = os.path.join(meta_dir,"test_tf_all_S2M_4views.txt")
  with open(train_meta_file, 'w') as file:
