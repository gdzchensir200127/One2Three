...
class Predictor(CheckpointRunner):
    ...
    def init_fn(self, shared_model=None, **kwargs):
        ...
        if self.options.model.name == "pixel2mesh":
            ...
            if self.gpu_inference:
                self.model.cuda()
                self.displayer = PtDisplayer(self.options.dataset.xlim,self.options.dataset.ylim,self.options.dataset.zlim)
    ...
    def predict_step(self, input_batch):
        ...
        with torch.no_grad():
            if self.options.model.name == "signal_refiner":
                signal_abs = input_batch["simulate_signal_abs"]

                # predict with model
                out = self.model(signal_abs)
    ...
    def save_inference_results(self, inputs, outputs): 
        if self.options.model.name == "pixel2mesh":
            ...
            batch_size = inputs["simulate_signal_abs"].size(0)
            for i in range(batch_size):
                origin_path = inputs["filepath"][i]
                save_path = origin_path.replace('shapenet_dataset', 'shapenet_dataset_final')
                phase = inputs["origin_phase"][i].cpu().numpy()
                signal_abs = outputs[i].cpu().numpy() / 100
                predict_signal = (signal_abs * np.exp(1j*phase)).astype(np.complex128)
                save_dir = os.path.dirname(save_path)
                if not os.path.exists(save_dir):
                    os.makedirs(save_dir)
                savemat(save_path, {'data':predict_signal})
                pass
