...
options.just_load_weights = False

options.dataset = edict()
options.dataset.name = "shapenet"
options.dataset.dir = "shapenet"
options.dataset.subset_train = "train_tf_airplane_S2M"
options.dataset.subset_eval = "test_tf_airplane_S2M"
options.dataset.subset_predict = "predict_signal_refiner"
options.dataset.camera_f = [248., 248.]
options.dataset.camera_c = [111.5, 111.5]
options.dataset.mesh_pos = [0., 0., 0.]
options.dataset.radar_pos = [0.,0.5,0.60]
options.dataset.xlim = [-0.2,0.2]
options.dataset.ylim = [-0.2,0.2]
options.dataset.zlim = [-0.2,0.2]
...
options.dataset.shapenet.p2s = False
options.dataset.shapenet.shapenet_json = "shapenet.json"
...
options.model = edict()
options.model.name = "signal2point"
...
options.model.se = False
options.model.trans_layers = 3
options.model.trans_atten_in_single_view = False #if intput the features of all views to transformerEncoder, is False
...
options.model.radar_pos = [0.,0.5,0.60]
options.model.bin_num = 9
options.model.downsample_factor = 10  # down sample signal
options.model.visibility_n = 5 # update visibility 5 times in P2S
options.model.encoder_shared = False
options.model.renderDepth = 0.781
options.model.outViewN = 1
options.model.finetune=False
...
options.loss.weights.chamfer = 1.
options.loss.weights.emd = 1.
options.loss.weights.chamfer_opposite = 1.
options.loss.weights.reconst = 0.
options.loss.downsample_factor = 10 # down sample signal
options.loss.weights.signal2pixel_xyz = 1.
options.loss.weights.signal2pixel_mask = 1.
options.loss.signal_refiner = 'mse'

options.train = edict()
options.train.num_epochs = 50
options.train.batch_size = 4
options.train.summary_steps = 50#当 Trainer 训练到第 summary_steps 的整数倍 epoch 时，Trainer.test() 会调用 Evaluator.evaluate()；
...
options.test.summary_steps = 50 #当 Trainer 训练到第 summary_steps 的整数倍 epoch 时，Trainer.test() 会调用 Evaluator.evaluate()；
...
options.optim.Dname = "adam"
options.optim.adam_beta1 = 0.9
options.optim.Dadam_beta1 = 0.9
options.optim.sgd_momentum = 0.9
options.optim.Dsgd_momentum = 0.9
options.optim.lr = 5.0E-5
options.optim.Dlr = 5.0E-6
options.optim.wd = 1.0E-6
options.optim.Dwd = 1.0E-6
options.optim.lr_step = [30, 45]
options.optim.Dlr_step = [30, 45]
options.optim.lr_factor = 0.1
options.optim.Dlr_factor = 0.1
options.optim.G_adversarial_weight = 1

options.discriminator = edict() 
