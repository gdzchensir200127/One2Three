...
# from models.layers.emd import earth_mover_distance

class S2PLoss(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        # dist_emd = earth_mover_distance(gt_coord,pred_coord,transpose=False)

        # emd_loss += torch.mean(dist_emd)

        chamfer_loss += self.options.weights.chamfer * (torch.mean(dist1) +
                                                            self.options.weights.chamfer_opposite * torch.mean(dist2))
        # normal_loss += self.normal_loss(gt_normal, idx2, pred_coord, self.edges)
        ...
        loss = chamfer_loss  
            # + self.options.weights.emd * emd_loss
