...
class ComplexEncoder(nn.Module):
  ...
  def __init__(self,options = None,):
    ...
    # FC
    self.fc_1 = LinearModule(5632,2048)
    self.fc_2 = LinearModule(2048,1024)
...
class Signal2Pixel_Model(nn.Module):
    '''
    ...
    '''
    def __init__(self, options = None):
        super(Signal2Pixel_Model,self).__init__()
        self.encoder = ComplexEncoder(options=options)
        self.decoder = Decoder(outViewN=1, outW=128, outH=128, renderDepth=1)
   
    def forward(self,signals):
        out = self.encoder(signals['signal_real'],signals['signal_imag'])
        XYZ, maskLogit = self.decoder(out)
...
if __name__ == '__main__':
  ...
  # decoder = Decoder()
  options = edict()
  model = Signal2Pixel_Model(options).to(device)
