...
def fuse3D(cfg, XYZ, maskLogit, fuseTrans_q, fuseTrans_t):
    ...
    Return:
     ...
     """
    batch_size,_,_,_ = XYZ.size()
    # 2D to 3D coordinate transformation
    invKhom = cfg.Khom2Dto3D.inverse() # [4x4]
    invKhomTile = invKhom.repeat([batch_size, cfg.outViewN, 1, 1]) #[B,V,4x4]

    # viewpoint rigid transformation
    q_view = fuseTrans_q # [V, 4]
    t_view = fuseTrans_t
    ...
    RtHomTile_view = RtHom_view.unsqueeze(0).repeat([batch_size, 1, 1, 1])
    ...
    ML = maskLogit.clone().reshape([batch_size, 1, -1]) # [B,1,VHW]
    XYZhom = get3DhomCoord(XYZ, cfg)  # [B,V,4,HW]
    XYZid = torch.matmul(RtTile, XYZhom)  # [B,V,3,HW]

    # fuse point clouds
    XYZid = XYZid.permute([0, 2, 1, 3]).reshape([batch_size, 3, -1]) #[B,3,VHW]
...
def render2D(cfg, XYZid, ML, renderTrans):  # [B,1,VHW]
    ...
    batch_size,_,_ = XYZid.size()
    # target rigid transformation
    q_target = renderTrans.reshape([batch_size * cfg.novelN, 4]) #[BN,4]
    t_target = torch.Tensor([0, 0, -cfg.renderDepth]) \
                    .repeat([batch_size * cfg.novelN, 1]) \
                    .float().to(cfg.device) # [BN,3]
    RtHom_target = transParamsToHomMatrix(q_target, t_target) \
                    .reshape([batch_size, cfg.novelN, 4, 4]) # [B,N,4,4]
    ...
    KupHomTile = KupHom.repeat([batch_size, cfg.novelN, 1, 1]) #[B,N,4,4]
    ...
    upNewiZMLCnt = torch.zeros([batch_size, cfg.novelN, 3,
    ...
    upNewiZMLCnt = upNewiZMLCnt.reshape([batch_size * cfg.novelN,
                                         3,
                                         cfg.H * cfg.upscale,
    ...
    newiZMLCnt = newiZMLCnt.reshape(
        [batch_size, cfg.novelN, 3, cfg.H, cfg.W])  # [B,N,3,H,W]
    ...
    upNewML = torch.zeros([batch_size, cfg.novelN, 1,
                           cfg.H*cfg.upscale, cfg.W*cfg.upscale]
    ...
    upNewML = upNewML.reshape([batch_size * cfg.novelN,
                               1,
                               cfg.H * cfg.upscale,
    ...
    newMaskLogitInvis = newML.reshape(
        [batch_size, cfg.novelN, 1, cfg.H, cfg.W])  # [B,N,H,W,1]
...
def get3DhomCoord(XYZ, cfg):
    batch_size,_,_,_ = XYZ.size()
    ones = torch.ones([batch_size, cfg.outViewN, cfg.outH, cfg.outW]) \
                .to(XYZ.device)
    XYZhom = torch.cat([XYZ, ones], dim=1) \
                  .reshape([batch_size, 4, cfg.outViewN, -1])\
                  .permute([0, 2, 1, 3])
    return XYZhom  # [B,V,4,HW]

def get3DhomCoord2(XYZ, cfg):
    batch_size,_,_,_ = XYZ.size()
    ones = torch.ones([batch_size, 1, cfg.outViewN * cfg.outH * cfg.outW]) \
                .to(XYZ.device)
    XYZhom = torch.cat([XYZ, ones], dim=1)
